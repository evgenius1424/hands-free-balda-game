      ...
      <CardContent className="space-y-4">
        {!isValid ? (
          <div className="flex items-center gap-2 p-3 bg-destructive/10 rounded-lg">
            ...
            <span className="text-sm text-destructive">Слово не является допустимым русским существительным</span>
          </div>
        ) : placements.length === 0 ? (
              ...
              <div key={index} className="p-3 bg-primary/10 rounded-lg border border-primary/20">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium">
                    {placement.direction === "horizontal" ? "Горизонтально" : "Вертикально"}
                  </span>
                ...
                <div className="text-xs text-muted-foreground mb-2">
                  Позиция: ({placement.startRow + 1}, {placement.startCol + 1})
                  <br />
                  Новая буква: {placement.newLetter} в ({placement.newLetterPos.row + 1},{" "}
                  {placement.newLetterPos.col + 1})
                </div>
